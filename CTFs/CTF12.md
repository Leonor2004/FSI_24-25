...
This task is done in the first step where we search for prime numbers above and below the candidates given to us and save them in a list to use after this task is done.

### Task 2

After we found the possible prime numbers used to encrypt the message we need to find the private exponent. This step is explained in the third step.

And then we have everything to decrypt the message, what is done in the step 4.

### Task 3

**Question1:** How can I use the information I have to infer the values used in the RSA that encrypted the flag?

**Answer1:** We know that the primes `p` and `q` are generated close to the numbers generated by the formulas: `2^500+(((t-1)*10+g)//2)` and `2^501+(((t-1)*10+g)//2)` respectively. This is done in the first step.

**Question2:** How can I find out if my inference is correct?

**Answer2:** To verify if any of the numbers we found is correct we just need to multiply the candidates (`p` and `q`) and verify if the result is equal to the `Modulos` that we were given. This is done in the second step.

**Question3:** How can i extract my key from the cryptogram i received?

**Answer3:** After finding the right `p` and `q` we first calculate the `phi(n)`. Use this number to calculate the private key and then decypher the message with this private key. As we have explain in the third and fourth step.